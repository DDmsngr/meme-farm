...
// Хэмминг-дистанция на 64-битном phash, ≤ значит визуальный дубль.
// 2026-09-06: эволюция 12→16→20 — НО этот период проверка была молча сломана
// (findVisualDuplicate падал на Map, см. фикс 2026-09-17), так что калибровка
// была вслепую и не отражает реальность. 2026-09-18: после починки порог 20
// оказался катастрофически мягким — с накопленными ~450 phash-записями в dedup
// он ловил почти ЛЮБОЙ новый мем как «дубль» (у 6 заведомо разных случайных фото
// в тесте минимальное расстояние было 21 из 64 бит — порог 20 уже внутри шума).
// Настоящий дубль (посты 1588/1589) имел расстояние 1. Понижено до 10 —
// стандартное значение для 64-бит phash, с большим запасом ловит дубли/пересжатия.
...
const AD_HARD_TRIGGERS = [
  ...
  // политика/война — hard skip (2026-09-03)
  { name: 'putin', re: /(?<!\p{L})путин\p{L}*(?!\p{L})/iu },
  { name: 'zelensky', re: /(?<!\p{L})зеленск\p{L}*(?!\p{L})/iu },
  { name: 'ukraine', re: /(?<!\p{L})(?:украин|киев|всу|киевск)\p{L}*(?!\p{L})/iu },
  { name: 'war', re: /(?<!\p{L})войн(?:а|ы|у|е|ой|ам|ах)?(?!\p{L})/iu },
  { name: 'shelling', re: /(?<!\p{L})обстрел\p{L}*(?!\p{L})/iu },
  { name: 'svo', re: /(?<!\p{L})сво(?!\p{L})/iu },
  { name: 'mobilization', re: /(?<!\p{L})мобилизац\p{L}*(?!\p{L})/iu },
  { name: 'donbass', re: /(?<!\p{L})(?:донбасс|днр|лнр|мариупол|луганск|донецк|харьков|херсон|запорожь|мелитопол|бахмут|авдеевк|артёмовск)\p{L}*(?!\p{L})/iu },
  { name: 'crimea', re: /(?<!\p{L})крым(?:а|у|е|ом)?(?!\p{L})/iu },
  { name: 'drone', re: /(?<!\p{L})(?:беспилотн|бпла|дрон)\p{L}*(?!\p{L})/iu },
  { name: 'front', re: /(?<!\p{L})(?:фронт(?:а|у|е|ом|ов|ах|овик)?|линия\s+фронта)(?!\p{L})/iu },
  { name: 'army-ru', re: /(?<!\p{L})(?:минобороны|мо\s+рф|вс\s+рф|армии\s+рф|российск(?:ая|ой|ую)\s+армия)/iu },
  { name: 'shoigu', re: /(?<!\p{L})(?:шойгу|герасимов|белоусов)(?!\p{L})/iu },
